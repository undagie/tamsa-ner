...
OUTPUT_DIR = _ROOT / "outputs" / "experiment_bilstm_w2v_cnn"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

BEST_MODEL_PATH = OUTPUT_DIR / "bilstm-w2v-cnn-crf-best.pt"
...
def collate_fn(batch):
    """Collate function for DataLoader to pad sequences including character sequences."""
...
class BiLSTM_W2V_CNN_CRF(nn.Module):
    ...
    def forward(self, words, tags, mask, chars):
...
def main():
    ...
    train_dataset = NERDataset(train_data, word_to_ix, tag_to_ix, char_to_ix)
    dev_dataset = NERDataset(dev_data, word_to_ix, tag_to_ix, char_to_ix)
    ...
    for epoch in range(HP["epochs"]):
        ...
        for batch_idx, (_, words, tags, mask, chars) in enumerate(train_loader):
            words, tags, mask, chars = words.to(DEVICE, non_blocking=True), tags.to(DEVICE, non_blocking=True), mask.to(DEVICE, non_blocking=True), chars.to(DEVICE, non_blocking=True)
            
            if scaler:
                with autocast(device_type='cuda'):
                    loss = model(words, tags, mask, chars)
    ...
    summary = {
        "model_name": "BiLSTM-CRF + Word2Vec + CharCNN",
