...
OUTPUT_DIR = _ROOT / "outputs" / "evaluation_nerui_bilstm"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

ORIGINAL_EXPERIMENT_DIR = _ROOT / "outputs" / "experiment_bilstm"
BEST_MODEL_PATH = ORIGINAL_EXPERIMENT_DIR / "bilstm-crf-best.pt"
...
print(f"Model: BiLSTM-CRF | Device: {DEVICE}")
...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim, lstm_layers, dropout):
    ...
    def predict(self, words, mask):
        emissions = self._get_lstm_features(words)
    ...
    with torch.no_grad():
        for tokens, words, tags, mask in loader:
            ...
            for i in range(len(preds)):
                ...
                pred_tags = [ix_to_tag.get(p, '<UNK>') for p in preds[i]] # preds is already a list of ints
...
def main():
    ...
    print("Initializing model BiLSTM-CRF...")
    model = BiLSTM_CRF(len(word_to_ix), tag_to_ix, HP["embedding_dim"], HP["lstm_hidden_dim"], HP["lstm_layers"], HP["dropout"])
    model.to(DEVICE)
