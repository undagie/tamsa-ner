...
print(f"Model: IndoBERT + BiLSTM-CRF | Device: {DEVICE}")
...
class IndoBERT_BiLSTM_CRF(nn.Module):
    """IndoBERT-BiLSTM-CRF model for Named Entity Recognition."""
    
    def __init__(self, tag_to_ix, hp):
        super(IndoBERT_BiLSTM_CRF, self).__init__()
        self.hp = hp
    ...
    with torch.no_grad():
        for tokens, bert_indices, bert_mask, tags, word_mask, word_maps in loader:
            ...
            for i in range(len(preds)):
                ...
                # BUG FIX: Using t.item() (int) and p (int) as keys, not strings
...
def main():
    ...
    # Convert ix_to_tag keys to integer because JSON stores them as strings
    ...
    print("Initializing IndoBERT-BiLSTM-CRF model...")
    model = IndoBERT_BiLSTM_CRF(tag_to_ix, HP).to(DEVICE)
    ...
    summary = {
        "model_name": "IndoBERT + BiLSTM-CRF",
